...
fn user_query(text: &str) -> Option<String> {
    ...
    let end = text[start..].find("</user_query>").map(|e| start + e).unwrap_or(text.len());
...
fn prompt_timestamp(text: &str) -> Option<i64> {
    ...
    let month = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].iter().position(|m| *m == mon)? as u32 + 1;
    ...
    let offset_min: i64 = rest
        .split("(UTC")
        .nth(1)
        .and_then(|z| z.trim_end_matches(')').split_once(':').map(|(h, m)| (h.to_string(), m.to_string())).or_else(|| Some((z.trim_end_matches(')').to_string(), "0".into()))))
        .and_then(|(h, m)| {
...
impl Provider for Cursor {
    ...
    fn log_roots(&self) -> Vec<PathBuf> {
        ...
        Self::home().map(|h| vec![h.join("projects")]).unwrap_or_default()
    }

    fn installed(&self) -> bool {
        Self::home().is_some_and(|h| h.join("projects").is_dir() || h.join("chats").is_dir() || h.join("cli-config.json").exists())
            || super::on_path("cursor-agent")
    }

    fn matches(&self, path: &Path) -> bool {
        path.extension().is_some_and(|e| e == "jsonl")
            && path.components().any(|c| c.as_os_str() == "agent-transcripts")
    ...
    fn parse_line(&self, path: &Path, line: &str) -> Result<Vec<Record>> {
        ...
        if st.session_id.is_empty() {
            st.session_id = path.file_stem().and_then(|s| s.to_str()).unwrap_or("").to_string();
            st.is_subagent = path.parent().and_then(|p| p.file_name()).is_some_and(|n| n == "subagents");
            // …/projects/<carpeta>/agent-transcripts/<id>/<id>.jsonl
            let mut anc = path.ancestors();
            let project_dir = anc.find(|a| a.parent().and_then(|p| p.file_name()).is_some_and(|n| n == "projects"));
            st.cwd = project_dir.and_then(|d| d.file_name()).and_then(|n| n.to_str()).and_then(cwd_from_project_dir);
        ...
        match v["role"].as_str() {
            Some("user") if !st.is_subagent => {
                let text: String = blocks.iter().filter(|b| b["type"] == "text").filter_map(|b| b["text"].as_str()).collect::<Vec<_>>().join(" ");
                let Some(query) = user_query(&text) else { return Ok(vec![]) }; // contexto inyectado, no un prompt
                ...
                out.push(Record::Turn(TurnRec { id, session_id: st.session_id.clone(), ts, intent: prompt_intent(&query) }));
            }
            Some("assistant") => {
                ...
                for b in &blocks {
                    match b["type"].as_str() {
                        Some("text") | Some("reasoning") => chars += b["text"].as_str().map(|t| t.len()).unwrap_or(0),
                        Some("tool_use") | Some("tool-call") => {
                            let raw = b["name"].as_str().or(b["toolName"].as_str()).unwrap_or("?");
                            let tool = canonical_tool(raw).to_string();
                            let args = if b["input"].is_object() { &b["input"] } else { &b["args"] };
                            chars += args.to_string().len();
                            let target = ["command", "path", "file_path", "target_file", "relative_workspace_path", "pattern", "query", "url"]
                                .iter()
                                .find_map(|k| args[*k].as_str())
                                .map(|s| s.chars().take(500).collect::<String>());
                            let call_id = b["id"].as_str().or(b["toolCallId"].as_str()).map(str::to_string).unwrap_or_else(|| format!("{message_id}:{}", out.len()));
                            out.push(Record::ToolUse(ToolUseRec {
                                ...
                                ts,
                                target,
                                detail: (tool == "Agent").then(|| args["subagent_type"].as_str().unwrap_or("general-purpose").to_string()),
                                tool,
                            }));
                        }
                        _ => {}
...
mod tests {
    ...
    fn consulta_y_fecha_del_prompt() {
        ...
        assert_eq!(prompt_timestamp(t), Some(chrono::NaiveDate::from_ymd_opt(2026, 9, 3).unwrap().and_hms_opt(13, 52, 0).unwrap().and_utc().timestamp_millis()));
        assert_eq!(user_query("<user_info>solo contexto</user_info>"), None);
    }

    #[test]
    fn carpeta_de_proyecto() {
        assert_eq!(cwd_from_project_dir("-home-dev-Proyectos-demo").as_deref(), Some("/home/dev/Proyectos/demo"));
    ...
    fn detecta_transcripciones() {
        ...
        assert!(p.matches(Path::new("/h/.cursor/projects/-h-x/agent-transcripts/abc/abc.jsonl")));
        assert!(p.matches(Path::new("/h/.cursor/projects/-h-x/agent-transcripts/abc/subagents/s.jsonl")));
